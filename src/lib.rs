...
use wgpu::{
    ...
    BindGroupLayoutDescriptor, BufferDescriptor, ComputePipelineDescriptor, Features, PipelineLayout, PushConstantRange, RenderPipelineDescriptor,
};
use winit::{
    dpi::{PhysicalPosition, PhysicalSize},
    error::{EventLoopError, OsError},
    event::{AxisId, DeviceId, ElementState, Event, Ime, InnerSizeWriter, KeyEvent, Modifiers, MouseButton, MouseScrollDelta, Touch, TouchPhase},
    event_loop::{EventLoop, EventLoopWindowTarget},
    window::{Fullscreen, Window, WindowBuilder, WindowButtons, WindowLevel},
};
...
pub struct HeatwaveRunner<'a, Handler>
where
    Handler: Presenter,
{
    presenter: Handler,
...
impl<'a, Handler> HeatwaveRunner<'a, Handler>
where
    Handler: Presenter + Send + 'static,
{
    ///The user side of the default handler, used by [`HeatwaveRunner::run`]
    ///
    ...
    pub fn default_handler_user(
        presenter: &mut Handler,
        mouse_position: &mut PhysicalPosition<f64>,
        event: WindowEvent,
    ) -> UserResponse<Handler::RenderData> {
        match event {
            WindowEvent::CursorEntered { device_id } => presenter.on_cursor_enter(device_id),
            WindowEvent::CursorMoved { device_id, position } => {
                presenter.on_cursor_move(device_id, position);
            ...
            WindowEvent::MouseDown { device_id, state, button } => presenter.on_mouse_down(MousePressEventArgs {
                device_id,
                state,
                button,
                position: *mouse_position,
            }),
            WindowEvent::MouseUp { device_id, state, button } => presenter.on_mouse_up(MousePressEventArgs {
                device_id,
                state,
                button,
                position: *mouse_position,
            ...
            WindowEvent::RequestRenderData => {
                return UserResponse::RenderDataPrepared(presenter.package_render_data());
            }
            WindowEvent::CloseRequested | WindowEvent::Destroyed => {
                presenter.on_exit();
                return UserResponse::ReadyToClose;
            }
    ...
    ///
    ///`sender` sends messages to the user thread, `receiver` receives messages back from the user thread.
    ///
    ///Public only for people using [`HeatwaveRunner::run_custom`] to use as a fallback/base implementation for their custom handlers
    pub fn default_handler_render(args: EventArgs, app: &HeatwaveApp, sender: &Sender<WindowEvent>, receiver: &Receiver<UserResponse<<Handler as Presenter>::RenderData>>) {
        match args.event {
            Event::WindowEvent { window_id, event } if window_id == app.window().id() => match event {
                winit::event::WindowEvent::RedrawRequested => {
                    if sender.send(WindowEvent::RequestRenderData).is_err() {
						error!("User thread disconnected early! Shutting down window (Failed to request render data)");
						args.target.exit();
						return;
					}
                    loop {
                        let response = receiver.recv_timeout(Duration::from_millis(400));
                        if let Ok(response) = response {
                            if let UserResponse::RenderDataPrepared(render_data) = response {
                                Handler::render(render_data, Some(RenderHelper::new(app)));
                                break;
                            }
                        } else {
                            error!("Presenter did not respond with render data, assuming it is dead and ending the app");
                            args.target.exit();
                            break;
                        }

                        warn!("Desync between render and user event thread! Ignoring non-render data response from user");
                    }
                }
                winit::event::WindowEvent::CloseRequested => {
                    ...
                    loop {
                        match receiver.recv_timeout(Duration::from_millis(400)) {
                            Err(_) => {
                                warn!("User thread did not respond to close request, assuming it is dead");
                                break;
                            }
                            Ok(message) => match message {
                                UserResponse::ReadyToClose => break,
                                _ => warn!("Desync between render and user event thread! Ignoring non-closing response from user"),
                            },
                        }
                    }
                }
                event => {
                    if sender.send(event.into()).is_err() {
						error!("User thread disconnected early! Shutting down window (Failed to send events)");
						args.target.exit()
					}
                }
            },
    ...
    pub fn run(mut self) -> Result<(), EventLoopError> {
        let (sender_user_to_render, receiver_user_to_render) = channel();
        let (sender_render_to_user, receiver_render_to_user) = channel();

        thread::spawn(move || {
            let mut mouse_position = PhysicalPosition::<f64>::new(0.0, 0.0);
            loop {
                match receiver_render_to_user.recv_timeout(Duration::from_millis(400)) {
                    Err(_) => {
                        error!("Main thread disconnected early! Shutting down user thread (Failed to poll for events)");
                        break;
                    }
                    Ok(event) => match Self::default_handler_user(&mut self.presenter, &mut mouse_position, event) {
                        UserResponse::Ok => {}
                        UserResponse::RenderDataPrepared(data) => {
                            if sender_user_to_render.send(UserResponse::RenderDataPrepared(data)).is_err() {
                                error!("Main thread disconnected early! Shutting down user thread (Failed to send render data)");
                                break;
                            }
                        }
                        UserResponse::ReadyToClose => {
                            if sender_user_to_render.send(UserResponse::ReadyToClose).is_err() {
                                warn!("Main thread disconnected early! Cleaning up user thread");
                                break;
                            }
                        }
                        _ => {
                            //This should only occur if the main thread dies or is unresponsive, so that this thread is cleaned up asap.
                            break; //Currently, we just kill the user thread on any of the errors it returns, since they're all fatal.
                        }
                    },
                };
            }
        }); //Will not handle panics. Shouldn't ever panic, anyway, as all error conditions are handled.
		//The thread won't perpetuate forever, as it times out after 400ms and closes the thread if no events are heard from the main thread

		let sender_ref = &sender_render_to_user;
		let receiver_ref = &receiver_user_to_render;

        self.event_loop.run(move |event, target| {
            Self::default_handler_render(EventArgs { event, target }, &self.app, sender_ref, receiver_ref);
        })
    }
    ...
    pub fn run_custom<HandleFn>(mut self, handler: HandleFn) -> Result<(), EventLoopError>
    where
        HandleFn: Fn(EventArgs, &HeatwaveApp, &mut Handler),
    {
        self.event_loop.run(move |event, target| {
            handler(EventArgs { event, target }, &self.app, &mut self.presenter);
        })
    }

	///Dissolves the runner into its basic components, in the event that features like run_custom is not appropriate for your use case.
	pub fn dissolve<'b>(self) -> (Handler, EventLoop<()>, HeatwaveApp<'b>) where 'a: 'b {
		(self.presenter, self.event_loop, self.app)
	}
}
...
pub enum UserResponse<RenderData> {
    Ok,
    PollFailed,
    FailedToSendRenderData,
    FailedToSendClose,
...
impl<'a> Default for HeatwaveConfig<'a> {
    fn default() -> Self {
        Self {
            ...
            bind_groups: &[],
            push_constants: &[],
            skybox: wgpu::Color { a: 1.0, r: 0.5, g: 0.5, b: 0.5 },
        }
    }
}
